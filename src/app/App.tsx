...
import { VacancyTabs } from '@/shared/ui/vacancyTabs';
...
function App() {
  ...
  useEffect(() => {
    ...
    dispatch(fetchVacancies({ city: selectedCity, skills, text: searchText }));
    setCurrentPage(1);
          ...
          <div>
            {currentVacancies.map((vacancy: Vacancy) => (
              <ul key={vacancy.id}>
                ...
                <li>
                  <button
                    onClick={() => navigate(`/vacancies/${city}/${vacancy.id}`)}
                  >
                    Смотреть вакансию
                  </button>
                  <a
